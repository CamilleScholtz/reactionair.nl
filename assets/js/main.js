...
const scroll = (header, stickies, mobile) => {
		...
		if (!mobile.matches) {
			stickies.forEach((sticky) => {
				if (sticky.parentNode.classList.contains("authors")) {
					sticky.style.top = `calc(${height}px + 55px + 40px + 1px - ${height}px)`;
				} else {
					sticky.style.top = `calc(${height}px + 55px + 50px + 1px - ${height}px)`;
				}
			});
		}
	...
	const show = () => {
		...
		if (!mobile.matches) {
			stickies.forEach((sticky) => {
				if (sticky.parentNode.classList.contains("authors")) {
					sticky.style.top = `calc(${height}px + 55px + 40px + 1px)`;
				} else {
					sticky.style.top = `calc(${height}px + 55px + 50px + 1px)`;
				}
			});
		}
